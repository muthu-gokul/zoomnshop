...
import '../hls-streaming/hls_broadcaster_page.dart';
import '../hls-streaming/hls_viewer_page.dart';
...
class _HLSScreenControllerState extends State<HLSScreenController> {
  ...
  void initMeeting() async {
    bool ans = await context.read<MeetingStore>().join(widget.user, widget.meetingLink);
    if (!ans) {
  ...
  Widget build(BuildContext context) {
   /* return HLSBroadcasterPage(
    ...
    if (Provider.of<MeetingStore>(context).localPeer != null && Provider.of<MeetingStore>(context).localPeer!.role.name.contains("hls-")) {
      return HLSViewerPage();
    }
    else {
      return HLSBroadcasterPage(
